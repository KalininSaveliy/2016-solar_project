...
def execution():
    ...
    При perform_execution == True функция запрашивает вызов самой себя по таймеру через
    время от 1 мс до 100 мс.
    """
    global physical_time
    global displayed_time
    recalculate_space_objects_positions(space, space_objects, time_step.get())
...
def open_file_dialog():
    ...
    max_distance = max([max(abs(obj.x), abs(obj.y))
                        for obj in space_objects])
...
def main():
    ...
    scale = tkinter.Scale(
        frame, variable=time_speed, orient=tkinter.HORIZONTAL)
    ...
    time_label = tkinter.Label(
        frame, textvariable=displayed_time, width=30)
